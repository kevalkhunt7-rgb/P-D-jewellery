...
export default function Home() {
  ...
  return (
    <div className="space-y-5 sm:space-y-6 max-w-7xl mx-auto p-1 text-slate-200">
      <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {/* Total Revenue */}
        <div className="bg-slate-900 border border-slate-800/80 rounded-2xl p-4 sm:p-5 shadow-sm">
          <div className="flex items-center justify-between pb-2">
            <span className="text-xs font-medium text-slate-400 tracking-wide">Total Revenue</span>
            <FiDollarSign className="w-4 h-4 text-amber-500 shrink-0" />
          </div>
          <div className="text-xl sm:text-2xl font-bold tracking-tight text-white truncate">
            ₹{stats.totalRevenue.toLocaleString()}
          </div>
          <div className="text-[11px] text-slate-400 flex items-center gap-1 mt-1.5 flex-wrap">
            <FiTrendingUp className="w-3 h-3 text-emerald-500 shrink-0" />
        ...
        <div className="bg-slate-900 border border-slate-800/80 rounded-2xl p-4 sm:p-5 shadow-sm">
          <div className="flex items-center justify-between pb-2">
            ...
            <FiShoppingCart className="w-4 h-4 text-amber-500 shrink-0" />
          </div>
          <div className="text-xl sm:text-2xl font-bold tracking-tight text-white truncate">
        ...
        <div className="bg-slate-900 border border-slate-800/80 rounded-2xl p-4 sm:p-5 shadow-sm">
          <div className="flex items-center justify-between pb-2">
            ...
            <FiUsers className="w-4 h-4 text-amber-500 shrink-0" />
          </div>
          <div className="text-xl sm:text-2xl font-bold tracking-tight text-white truncate">
        ...
        <div className="bg-slate-900 border border-slate-800/80 rounded-2xl p-4 sm:p-5 shadow-sm">
          <div className="flex items-center justify-between pb-2">
            ...
            <FiPackage className="w-4 h-4 text-amber-500 shrink-0" />
          </div>
          <div className="text-xl sm:text-2xl font-bold tracking-tight text-white truncate">
            {stats.productsSold.toLocaleString()}
          </div>
          <div className="text-[11px] text-slate-400 flex items-center gap-1 mt-1.5 flex-wrap">
            <FiTrendingDown className="w-3 h-3 text-rose-500 shrink-0" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-7 gap-4">
        {/* Revenue Analytics */}
        <div className="bg-slate-900 border border-slate-800/80 rounded-2xl p-4 sm:p-5 lg:col-span-4">
          <div className="mb-4">
          ...
          <div className="w-full h-[220px] sm:h-[260px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={revenueData} margin={{ left: -15, right: 5, top: 10, bottom: 0 }}>
                <defs>
        ...
        {/* Sales by Category */}
        <div className="bg-slate-900 border border-slate-800/80 rounded-2xl p-4 sm:p-5 lg:col-span-3">
          <div className="mb-4">
          ...
          <div className="w-full h-[220px] sm:h-[260px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={categoryData} margin={{ left: -15, right: 5, top: 10, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" vertical={false} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Recent Orders */}
        <div className="bg-slate-900 border border-slate-800/80 rounded-2xl p-4 sm:p-5 overflow-hidden">
          <div className="mb-4">
            <h2 className="text-sm font-semibold text-white">Recent Orders</h2>
            <p className="text-[11px] text-slate-400 mt-0.5">Incoming purchases ledger</p>
          </div>

          {/* Mobile: stacked cards (avoids a cramped horizontal-scroll table) */}
          <div className="space-y-2.5 sm:hidden">
            {recentOrders.map((order) => (
              <div key={order._id} className="rounded-xl border border-slate-800/80 bg-slate-800/20 p-3">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-1.5 font-semibold text-amber-500/90 text-xs">
                  ...
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium tracking-wide border uppercase shrink-0 ${getStatusStyles(order.orderStatus)}`}>
                    {order.orderStatus}
                  </span>
                </div>
                <div className="mt-2 flex items-end justify-between gap-2">
                  <div className="min-w-0">
                    <p className="font-medium text-xs text-slate-200 truncate">{order.user?.name || 'Guest'}</p>
                ...
                {recentOrders.map((order) => (
                  <tr key={order._id} className="hover:bg-slate-800/20 transition-colors">
                    ...
                    <td className="py-3.5">
                      ...
                      <div className="text-[10px] text-slate-500 mt-0.5 max-w-[140px] lg:max-w-[180px] truncate">
                        {order.orderItems.map(item => item.name).join(", ")}
                      </div>
                    </td>
        ...
        <div className="bg-slate-900 border border-slate-800/80 rounded-2xl p-4 sm:p-5">
          ...
          <div className="space-y-3 sm:space-y-4">
            {topProducts.map((product, index) => (
              <div key={index} className="flex items-center gap-3 sm:gap-3.5 p-1 hover:bg-slate-800/10 rounded-xl transition-all">
                <img
                  src={getProductImage(product)}
                  alt={product.name}
                  className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl object-cover border border-slate-800 flex-shrink-0"
                />
        ...
        <div className="bg-slate-900 border border-slate-800/80 rounded-2xl p-4 sm:p-5">
          ...
          <div className="space-y-2.5">
            {lowStockItems.map((item, index) => (
              <div
                key={index}
                className="flex items-center justify-between gap-2 p-3 rounded-xl bg-amber-500/5 border border-amber-500/10"
              >
                ...
                <span className="px-2 py-0.5 rounded-md text-[10px] font-semibold bg-amber-500/10 border border-amber-500/20 text-amber-400 whitespace-nowrap shrink-0">
        ...
        <div className="bg-slate-900 border border-slate-800/80 rounded-2xl p-4 sm:p-5">
          ...
          <div className="space-y-3 sm:space-y-4">
            {recentCustomers.map((customer, index) => (
              <div key={index} className="flex items-center gap-3 sm:gap-3.5 p-1">
